...
/* Current generation of worker processes.  after a sighup prior
...
ssl_read(struct ev_loop *loop, ev_io *w, int revents)
{
    ...
    if (t > 0) {//printf("%s\n",buf);
        ...
        ret = process_mqtt(buf,t,ps->req);
        if(REQ_CNT == ret)
           req_stat(ps->remote_ip, 1, 0);
        if(DROP == ret || ALERT == ret)
        {
            log_mqtt_to_file(ps->remote_ip,ps->req,ret,buf,t);
            if(DROP == ret)
            {            
                shutdown_proxy(ps, SHUTDOWN_HARD);
                return;
        ...
        if(DROP == ret || ALERT == ret)
        {
            log_http_to_file(ps->remote_ip,ps->req,ret,buf,t);
            if(DROP == ret)
            {
...
handle_accept(struct ev_loop *loop, ev_io *w, int revents)
{
...
#ifdef HAVE_MQTT
    ps ->req = calloc(1, sizeof(mqtt_waf_msg));
#else
    ps ->req = calloc(1, sizeof(http_waf_msg));
#endif
    if(ps ->req == NULL)
    {
        (void)close(client);
        free(ps);
        ERR("{malloc-req-err}: %s\n", strerror(errno));
        return;
    }
    ...
    ringbuffer_init(&ps->ring_ssl2clear, CONFIG->RING_SLOTS,
        CONFIG->RING_DATA_LEN);
    ps->req->buf = ps->ring_ssl2clear.buf;
    /* set up events */
